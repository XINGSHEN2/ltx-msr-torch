...
# Source file: comfy/ldm/lightricks/vocoders/vocoder.py
...
from .torch_compat import cast_to, disable_weight_init
import numpy as np
import math
...
class LowPassFilter1d(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.conv1d(x, cast_to(self.filter.expand(C, -1, -1), dtype=x.dtype, device=x.device), stride=self.stride, groups=C)
...
class Snake(nn.Module):
    ...
    def forward(self, x):
        a = cast_to(self.alpha.unsqueeze(0).unsqueeze(-1), dtype=x.dtype, device=x.device)
...
class SnakeBeta(nn.Module):
    ...
    def forward(self, x):
        a = cast_to(self.alpha.unsqueeze(0).unsqueeze(-1), dtype=x.dtype, device=x.device)
        b = cast_to(self.beta.unsqueeze(0).unsqueeze(-1), dtype=x.dtype, device=x.device)
        if self.alpha_logscale:
            a = torch.exp(a)
            b = torch.exp(b)
...
class _STFTFn(nn.Module):
    ...
    def forward(self, y: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        spec = F.conv1d(y, cast_to(self.forward_basis, dtype=y.dtype, device=y.device), stride=self.hop_length, padding=0)
...
class MelSTFT(nn.Module):
    ...
    def mel_spectrogram(
        ...
        mel = torch.matmul(cast_to(self.mel_basis, dtype=magnitude.dtype, device=y.device), magnitude)
