...
模型连接、IC-LoRA 引导注入、采样、VAE 解码及冒烟视频写入等环节的本地
PyTorch 替代实现。ComfyUI 仍用于一致性参考和 API prompt 对比，但主要重构
代码在本地 torch 环境中运行。
...
本实现不会内置 ComfyUI、自定义节点、模型、LoRA 或文本编码器权重。这些外部
组件分别受其自身许可证和使用条款约束。重新分发或商业使用前，请检查相关
ComfyUI/自定义节点项目以及 LTX-2.3、Gemma 文本编码器和
LTX-2.3-Licon-MSR-V1 模型资源的许可证。
...
3. 继续减少 VAE 路径中剩余的 ComfyUI 源码依赖。
...
解码器目前从 ComfyUI 导入 LTX VAE 类。请将 ComfyUI 克隆到项目旁边（也可以
放在其他目录），并安装其依赖：
...
文本编码器还需要 `gemma3cfg.json`、`tokenizer.json`、`tokenizer.model` 和
`tokenizer_config.json`。这些文件位于
[ComfyUI-LTXVideo 的 `gemma_configs` 目录](https://github.com/Lightricks/ComfyUI-LTXVideo/tree/master/gemma_configs)：
...
要把项目内样例提交给 ComfyUI，先在 ComfyUI 输入目录中暴露本项目：
